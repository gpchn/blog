...
如果你看到这两串字笑了，说明你亲手跟编码问题搏斗过。如果你没笑，那恭喜你——现代软件自动帮你擦了屁股而你浑然不知。

这两串乱码，是字符编码战争留给现代程序员的墓碑。"锟斤拷"是编码转换产生的不可逆错乱，而"烫烫烫"是 VC++ 的 debug 模式在未初始化堆内存里填的 0xCC，被当作汉字解码后的结果。前者是编码猜错的恶果，后者是内存越界的警报——两行乱码，两个时代的噩梦。

它们都涉及到一种文件类型，即纯文本，`.txt`。很多披着外壳的扩展名，如 `.md`、`.html`、`.json`，还有各种编程语言的源代码 `.py`、`.c`、`.rs`，其实也都是纯文本。

一个 `.txt` 文件，本质上就是一堆字节。它没有告诉你它用的是什么编码，没有头部，没有元数据，没有自我声明。它就这么赤裸裸地把一堆二进制数甩给你，然后指望你能猜对它的心思。

从 1960 年代起，一直到 2026 年还有人因为编码问题炸毛。这是一个关于妥协和无奈的故事。
...
ASCII 用 **7 位二进制数**表示一个字符，一共 128 个位置。0-31 是控制字符（回车、换行、响铃之类的），32 是空格，33-126 是可见字符——大小写英文字母、数字、标点符号。127 是 DEL。
...
这就是所谓的 **Mojibake**（日文"文字化け"），在中文世界里有个更亲切的名字：**乱码**。
...
中文上电脑，这个问题困扰了一代中国程序员。经过前人长期摸索，1980 年，中国国家标准总局发布了 **GB 2312-80**——《信息交换用汉字编码字符集·基本集》。这套编码采用双字节方案：每个汉字用两个字节表示，每个字节的高位都是 1（即值大于 127），这样就能与 ASCII 区分开。
...
1992 年 9 月 2 日的晚上，Plan 9 操作系统的开发者 Ken Thompson 和 Rob Pike 在 New Jersey 的一家饭店里，用了一个晚上设计出了 UTF-8。

![Ken Thompson](7.webp)

当时 Unicode 已经发布，但 Plan 9 团队需要一个高效的编码方案。他们讨论了几种可能性，最后 Thompson 随手拿起桌上的餐垫，在背面画出了 UTF-8 的核心设计。这个在餐垫上诞生的编码方案，后来统治了整个互联网。
...
这就是著名的 **UTF-8 BOM 问题**。它不是一个 bug，它是一个历史遗留设计，但它在 2026 年依然能浪费开发者半天时间。
...
从 ASCII 到 Unicode，从 GB 2312 到 UTF-8，编码标准已经发展了六十年。按理说这问题早该解决了。但现实是：**2026 年了，你依然可能收到一份乱码的 `.txt` 文件。**
...
这种设计有一个专业术语：**"什么都不承诺"**。这是一种设计哲学。

在 1960 年代，这是务实的——文件系统能提供的元数据就那么点，把编码信息写进文件本身会浪费宝贵的存储空间。在 2026 年，这是历史遗留的沉重包袱——我们有足够的空间存储元数据，但 `.txt` 的格式已经固化到无法改变。
