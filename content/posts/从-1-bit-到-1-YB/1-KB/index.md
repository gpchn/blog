...
上一回我们聊完了 1 Byte——那 8 个排队站好的比特，刚好够装下一个英文字母。计算机终于学会了咬下它的第一口数据。

但问题来了：一个字母能干什么？

什么也干不了。就好像你没办法只用一个汉字写完一首诗，计算机也没办法只用一个字节存下任何有价值的东西。它需要一个更大的单位。

这个单位叫 **Kilobyte**，译作 **千字节**。
...
这听起来只是技术细节的差异，但在实际工作中可以引发灾难性的后果。最出名的例子发生在 1983 年，加拿大航空公司的一架波音 767 客机在执行从蒙特利尔飞往埃德蒙顿的航班时，飞到一半突然双引擎熄火——整架飞机变成了一个 60 吨的滑翔机。后来调查发现，地勤给飞机加油时，把燃油量的"磅"和"公斤"搞混了。飞机实际只加了一半都不到的油。同样一桩混乱，如果把"KB"究竟是 1000 还是 1024 搞错，一个控制系统读出 1000 KB 的数据，另一个系统却以 1024 KB 来理解，偏差滚到 GB、TB 级别就会是天文数字级别的误判。

于是工程师们想了个临时的土办法：用小写 **k** 表示 1000，大写 **K** 表示 1024。kB 是按国际单位制的规矩来（1000 字节），KB 才是计算机世界的黑话（1024 字节）。这个约定在当年那个"写程序的人彼此都认识"的小圈子里，行得通——大家都心照不宣。
